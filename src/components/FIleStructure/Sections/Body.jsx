...
import { addChildren } from "@/redux/slices/fileStructureSlice"
...
export default function Body() {
  ...
  const { documents } = useSelector((state) => state.fileStructure)
  const [addedParentId, setAddedParentId] = useState(null)
  function handleAdd() {
    if (!folderName) {
    ...
    setError("")
    dispatch(addChildren({ name: folderName, parentId: addedParentId }))
    setFolderName("")
    setShowModal(false)
  }
  ...
  function generateFolder(data) {
    return (
      <div
        key={data.id}
        className="flex items-center gap-5 justify-between bg-gray-100 p-1 px-2 border-b"
      >
        ...
        <i
          className="fa fa-plus text-xs h-fit p-2 text-cyan-600 cursor-pointer"
          aria-hidden="true"
          onClick={() => {
            setAddedParentId(data.id)
            setShowModal(true)
          }}
        ></i>
      </div>
  ...
  function generateFile(data) {
    return (
      ...
      >
        <div className="flex items-center gap-2 cursor-pointer">
          <i className="fa-regular fa-file w-6"></i>
          <h1>{data.name}</h1>
        </div>
      </div>
    )
  }
  function generateType(doc) {
    let text = ""
    text = doc.type === "folder" ? generateFolder(doc) : generateFile(doc)
    return text
  }

  return (
    <section className="py-4 md:py-6 lg:py-8 px-4 md:px-6">
      <div className="grid">
        {documents.map((doc) => {
          return generateType(doc)
        })}
      </div>
      {/* modal */}
      {showModal && (
